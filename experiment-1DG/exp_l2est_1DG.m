...
for d = 1:nD
    ...
    for r = 1:nR
        ...
        Zy_n = sampleDist1D(pT_yn,MT,round(M.*pY(-1)),ulT);
        Zy_p = sampleDist1D(pT_yp,MT,round(M.*pY(+1)),ulT);
        
        % Rejection sampling of source data
        MS = 1./sqrt(2*pi*sigma2_S);
        Sy_n = sampleDist1D(pS_yn,MS,round(N.*pY(-1)),ul);
        Sy_p = sampleDist1D(pS_yp,MS,round(N.*pY(+1)),ul);
        ...
        if strcmp(ktype, 'lin')
            ...
            eta = @(b,l) (K(S,S,nu(b)) + Lambda(l)*eye(N))\yS;
