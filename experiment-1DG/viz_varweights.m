...
%% Variance of weights for arbitrary mu_S 
...
% Variance function
% Vw = @(g) real( 1./(2*sqrt(2*g.^2 - 1)) .* exp(1./(2*g.^2-1)) .* (2 - erf(real(1./(g.*sqrt(4*g.^2-2)))) + erf(real(1./sqrt(4*g.^4 - 2*g.^2)))) -1 );
...
set(gca, 'XScale', 'lin', 'XLim', [gamma(1) gamma(end)]);
set(gca, 'YScale', 'log', 'YLim', [1e0 1e2]);
